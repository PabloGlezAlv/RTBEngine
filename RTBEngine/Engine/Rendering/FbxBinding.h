    ...
    namespace Rendering {
        ...
        inline FbxBindingResult BuildMeshesAndMaterials(const FbxBindingContext& ctx)
        {
            ...
            for (size_t i = 0; i < ctx.modelData.embeddedTextures.size(); i++) {
                ...
                Texture* tex = nullptr;
                std::filesystem::path texturePath = fbxDir / (name + ".texture");
                if (std::filesystem::exists(texturePath)) {
                    std::string assetPath = texturePath.string();
                    for (char& c : assetPath) if (c == '\\') c = '/';
                    tex = ctx.resources.LoadTextureAsset(assetPath);
                }

                // Also check with .png/.jpg extensions for .texture file
                if (!tex) {
                    for (const char* ext : { ".png", ".jpg" }) {
                        std::filesystem::path imgTexPath = fbxDir / (name + ext);
                        imgTexPath.replace_extension(".texture");
                        if (std::filesystem::exists(imgTexPath)) {
                            std::string assetPath = imgTexPath.string();
                            for (char& c : assetPath) if (c == '\\') c = '/';
                            tex = ctx.resources.LoadTextureAsset(assetPath);
                            if (tex) break;
                        }
