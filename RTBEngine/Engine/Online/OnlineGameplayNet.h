...
namespace RTBEngine {
    namespace Online {
        ...
        public:
            ...
            static constexpr std::uint32_t kInvalidNetworkObjectId = 0;

            struct TransformSnapshot {
                std::uint32_t networkId = kInvalidNetworkObjectId;
            ...
            static void ResetNetworkSession();

            static std::uint32_t AllocateNetworkObjectId();
            static bool RegisterNetworkObjectId(std::uint32_t networkId);
            static void UnregisterNetworkObjectId(std::uint32_t networkId);
            static bool IsNetworkObjectIdRegistered(std::uint32_t networkId);

            static bool BroadcastTransform(const TransformSnapshot& snapshot);
            static bool TryGetLatestTransform(std::uint32_t networkId, TransformSnapshot& outSnapshot);
        };
