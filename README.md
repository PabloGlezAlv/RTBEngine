...
|----------|-------|--------|
| Engine semver | `1.0.0` | `Engine/Core/Version.h` → `RTBEngine::Core::VersionInfo` |
...
│   │       └── TypeInfoBuilder.h
│   │
│   ├── Assets/                    Runtime assets (models, prefabs, scenes, scripts, textures, …)
│   │   ├── Prefabs/               Player/Gameplay|Preview, Enemies, Combat/…
│   │   └── Scripts/               GameScripts sources by role (Player, Enemy, UI, …)
...
// Full FBX import for skinned characters (skeleton + meshes + embedded clips):
struct ModelData {
    std::shared_ptr<Animation::Skeleton> skeleton;
    std::vector<Rendering::Mesh*>        meshes;
    std::vector<std::shared_ptr<Animation::AnimationClip>> clips;
};
ModelData LoadModelData(const std::string& path);

// Animation-only FBX (e.g. KayKit packs) — clips only, no mesh registration:
std::vector<std::shared_ptr<Animation::AnimationClip>> LoadAnimationClips(const std::string& path);

std::string GetMeshPath(const Rendering::Mesh* mesh) const;
```

`LoadModelData` is used by `Animator::ReloadClipLibrary()` for the primary `modelRef` (rig + skinned meshes + clips from one FBX). `LoadAnimationClips` is used for `additionalModels` and for `LoadClipFromFbx` aliases — it parses animation data without caching or registering meshes, avoiding duplicate mesh entries when a character shares the same rig across multiple animation packs.
...
identity->SetNetworkPlayerSlot(0);  // host = slot 0
identity->SetOwnerUserId(OnlineSystem::GetInstance().GetIdentity()->GetLocalUserId());
```

**Runtime queries:**

```cpp
bool IsLocallyControlled() const;  // camera + input on this machine
bool IsSimulatedByHost() const;    // host runs movement/physics in online mode
std::string GetNetworkObjectKey() const;  // e.g. "PlayerSlot_0"
```

**Reflected properties:**

```cpp
std::string networkOwnerUserId;  // "Local:Player" or "NetworkPeer:192.168.0.2:27015"
...
```cpp
auto* netTransform = pawn->AddComponent<NetworkTransform>();
netTransform->sendRate = 20.0f;
netTransform->interpolationSpeed = 14.0f;
```
...
| Role | Phase | Action |
|------|-------|--------|
| Host | `OnFixedUpdate` | Sends transform at `sendRate` Hz via `OnlineGameplayNet::BroadcastTransform` |
| Client | `OnLateUpdate` | Reads latest snapshot and lerps/slerps toward it |

Incoming packets are drained once per frame by `OnlineSystem::Tick` (which calls `OnlineGameplayNet::Pump`).

**Reflected properties:**

```cpp
std::string objectKey;           // optional; defaults to NetworkIdentity key
...
The `ResourceManager` exposes this via `LoadModelData` (full rig) and `LoadAnimationClips` (clips only). `Animator::ReloadClipLibrary()` is the single consumer of both paths at runtime.
...
`Engine/Animation/Animator.h` — Skeletal animation component. Owns the clip library, drives bone transforms for GPU skinning, and optionally spawns bone `GameObject`s for debugging or attachment points.

**Clip loading (single pipeline):**

All clips are loaded through one path — there is no separate “validate” vs “configure” loader:

| API | Role |
|-----|------|
| `ReloadClipLibrary()` | Loads skeleton, meshes, and clips from `modelRef` + `additionalModels`; idempotent when paths are unchanged |
| `NormalizeClipName(raw)` | Strips vendor prefixes (`mixamo.com\|Walk` → `Walk`) |
| `LoadClipFromFbx(alias, fbxPath)` | Registers clips from an animation-only FBX under a stable alias (e.g. `ThirdPerson.Idle`); preserved across reloads |
| `EnsureSourcesLoaded()` | Private; called from lifecycle — compares `loadedPrimaryPath` / `loadedAdditionalPaths` |

Primary rig: `ResourceManager::LoadModelData(modelRef)`. Extra packs: `ResourceManager::LoadAnimationClips(path)` (no mesh cache pollution).
...
- `OnAwake` / `OnStart`: `EnsureSourcesLoaded()`; play `defaultClip` or `currentClipName` when appropriate (does not pick a random clip if an alias is pending).
- `OnUpdate`: advances time, samples clip, fills `finalBoneTransforms`.
- `OnValidate`: reloads when `modelRef` or `additionalModels` change; may create bone GameObjects in the **active** scene (known limitation in prefab edit mode).

**Scene / editor integration:**

- `SceneComponentConfigurator::ConfigureAnimator` calls `ReloadClipLibrary()` after serialized paths are applied.
- Editor Inspector delegates to `ReloadClipLibrary()` when animation asset fields change.

Clips live in `unordered_map<std::string, std::shared_ptr<AnimationClip>>`. FBX-derived names are tracked in `sourceDerivedClipNames` so reloads do not overwrite manual aliases from `LoadClipFromFbx`.
...
components = {
    ...
    { type = "NetworkTransform", sendRate = 20.0, interpolationSpeed = 14.0 },
}
```
